...
const NavigationMenuButton = ({ menuItem }: Props) => {
  ...
  const wrapperBgClass = selected ? "bg-warning" : "bg-white";
  const textClass = selected ? "text-white" : "text-warning";
  ...
  return (
    <Link key={menuItem.type} href={menuItem.url}>
      <div className={`rounded-md py-2 px-2 md:px-4 flex gap-8 transition-all duration-200 ${wrapperBgClass}`}>
        <div className={`${textClass} transition-all duration-200`}>{icon}</div>
        <div className={`${textClass} transition-all duration-200 uppercase hidden md:block`}>{menuItem.label}</div>
      </div>
    </Link>
  );
};
